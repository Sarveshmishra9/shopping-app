...
export default function CategoryFilterPicker() {
  ...
  const handleOnClick = (event) => {
    ...
    const newCategories = currSetCategories.includes(id)
      ? currSetCategories.filter((category) => category !== id)
      : [...currSetCategories, id];
        ...
        <div className="flex flex-col space-y-2 py-2">
          {categories.map((category) => (
            <div className="flex items-center space-x-2" key={category}>
              <Checkbox
                defaultChecked={inCurrSetCategories(category) ? true : false}
                id={category}
